...
(function() {
  var CSVReporter, CoffeeScript, ErrorReport, JSLintReporter, Reporter, coffeelint, config, data, errorReport, existsFn, findCoffeeScripts, fs, glob, lintFiles, lintSource, loadRules, optimist, options, path, paths, read, reportAndExit, ruleName, scripts, stdin, thisdir, _ref, _ref1, _ref2,
    __slice = [].slice,
  ...
  lintFiles = function(paths, config) {
    var errorReport, literate, source, _i, _len;
  ...
  loadRules = function(moduleName, ruleName) {
    var e, rule, ruleModule, _results;
  ...
  if (options.argv.v) {
    ...
    if (!options.argv.noconfig) {
      existsFn = (_ref2 = fs.existsSync) != null ? _ref2 : path.existsSync;
      if (options.argv.f) {
